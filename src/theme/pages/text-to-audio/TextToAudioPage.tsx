...
const displaimer =
  "Ecribe el texto que deseas convertir a audio y elige una de las voces disponibles. Todo el audio es generado por IA*.";
...
const TextToAudioPage = () => {
  ...
  const handlePostMessage = async (message: string, selectedVoice: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        text: `${selectedVoice} - ${data.messageToAudio}`,
        isGpt: true,
        type: "audio",
        audio: data.audioUrl,
      },
